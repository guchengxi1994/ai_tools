...
mod magika_tests {
    ...
    static CLASSES: Lazy<HashMap<String, u32>> = Lazy::new(|| {
    ...
    impl MagikaFile {
        pub fn new(path: String) -> anyhow::Result<Self> {
            ...
            if file_meta.len() < 512 {
                let mut file_content = vec![0; file_meta.len() as usize];
                file.read_exact(&mut file_content)?;
                let mut file_content_new: Vec<u32> =
                    file_content.iter().map(|&x| x as u32).collect();
                file_content_new.resize(512, 256);
                Ok(Self {
                    head: file_content_new.clone(),
                    middle: file_content_new.clone(),
                    end: file_content_new.clone(),
                ...
                let middle_start = (file_meta.len() / 2) as i64 - 256;
                file.seek(SeekFrom::Start(middle_start as u64))?;
                let mut middle_buffer = vec![0; 512];
                file.read_exact(&mut middle_buffer)?;

                let tail_start = file_meta.len().saturating_sub(512) as i64;
                file.seek(SeekFrom::Start(tail_start as u64))?;
                let mut tail_buffer = vec![0; 512];
                file.read_exact(&mut tail_buffer)?;

                Ok(Self {
                    head: header_buffer.iter().map(|&x| x as u32).collect(),
                    middle: middle_buffer.iter().map(|&x| x as u32).collect(),
                    end: tail_buffer.iter().map(|&x| x as u32).collect(),
                    r#type: ext.to_lowercase(),
                })
            }
        }
    ...
    impl Module for Magika {
        fn forward(&self, xs: &Tensor) -> candle_core::Result<Tensor> {
            ...
            for i in 0..batch_size {
                ...
                let x = one_hot(&x, 257, candle_core::DType::U32).unwrap();
    ...
    fn one_hot_test() -> anyhow::Result<()> {
        let d = Device::Cpu;
        let labels = Tensor::from_vec([0u32, 2u32, 1u32, 3u32].to_vec(), &[4], &d)?;
    ...
    fn magika_test() -> anyhow::Result<()> {
        ...
        let tensor = Tensor::ones(&[1536], candle_core::DType::U32, &device)?;
    ...
    fn test_magika_file() -> anyhow::Result<()> {
        ...
        let file = MagikaFile::new(
            r"C:\Users\xiaoshuyui\Desktop\屏幕截图 2024-10-02 211700.png".to_owned(),
        )?;
        let x = file.to_dataset(&device)?;
        println!("x shape: {:?}", x.0.shape());
        println!("y shape: {:?}", x.1.shape());
        println!("y value: {:?}", x.1.to_vec2::<u32>()?);
        let vm = VarMap::new();
        let vs = VarBuilder::from_varmap(&vm, candle_core::DType::F32, &device);
